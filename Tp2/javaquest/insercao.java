...
public class insercao {
    public static void main(String[] args) throws IOException {
        ...
        int[] temp = insertionSortByYear(players);
    ...
    public static int[] insertionSortByYear(LinkedList<Jogador> jogadores) {
        int comparacoes = 0;
        int movimentacoes = 0;
        
        for (int j = 1; j < jogadores.size(); j++) {
            Jogador key = jogadores.get(j);
            int i = j - 1;
    
            while (i >= 0 && (jogadores.get(i).getAnoNascimento() > key.getAnoNascimento() ||
                    (jogadores.get(i).getAnoNascimento() == key.getAnoNascimento() &&
                    jogadores.get(i).getNome().compareTo(key.getNome()) > 0))) {
                jogadores.set(i + 1, jogadores.get(i));
                i--;
                comparacoes++;
                movimentacoes++;
            }
            jogadores.set(i + 1, key);
            movimentacoes++;
        }
